...
% Copyright 2016 The MathWorks, Inc.
  ...
  methods
    function this = circularGraphEditted(adjacencyMatrix,varargin)
      ...
      addRequired(p,'adjacencyMatrix',@(x)(isnumeric(x) || islogical(x))); % RED FLAG islogical turns matrix to 1 and 0
      addParameter(p,'ColorMap',defaultColorMap,@(colormap)length(colormap) == length(adjacencyMatrix)); % Verify length of colormap (num colors) = number inputs from matrix
