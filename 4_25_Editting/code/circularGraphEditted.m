classdef circularGraphEditted < handle
% circularGraph Plot an interactive circular graph to illustrate connections in a network.
%
%% Syntax
% circularGraphEditted(X)
% circularGraphEditted(X,'PropertyName',propertyvalue,...)
% h = circularGraphEditted(...)
%
%% Description
...
% Copyright 2016 The MathWorks, Inc.
  properties
    Node = node(0,0); % Array of nodes, all zeros
    ColorMap;         % Colormap
  ...
  methods
    function this = circularGraphEditted(adjacencyMatrix,varargin)
      % Constructor
      p = inputParser;
      largestValueMatrix = largestValue(adjacencyMatrix);
      largestValueMatrix = round(largestValueMatrix)
      
      defaultColorMap = jet(largestValueMatrix)    % Set numbr of colors used
      defaultLabel = cell(length(adjacencyMatrix)); % Set size of array to hold labels
      for i = 1:length(defaultLabel)
        defaultLabel{i} = num2str(i);               % Make labels, strings instead of integers
      end
      
      addRequired(p,'adjacencyMatrix',@(x)(isnumeric(x))); % Check if values are numbers
      addParameter(p,'ColorMap',defaultColorMap,@(colormap)length(colormap) == largestValueMatrix); % Verify length of colormap (num colors) = highest value in matrix
      ...
      this.ShowButton = uicontrol(...
        ...
        'Position',[0 40 80 40],...
        'String','Show All',...
        'Callback',@circularGraphEditted.showNodes,...
        'UserData',this);
      ...
      this.HideButton = uicontrol(...
        ...
        'Position',[0 0 80 40],...
        'String','Hide All',...
        'Callback',@circularGraphEditted.hideNodes,...
        'UserData',this);
      
      fig = gcf;
      set(fig,...
        'UserData',this,...
        'CloseRequestFcn',@circularGraphEditted.CloseRequestFcn);
      ...
      for i = 1:length(adjacencyMatrix)
        ...
        this.Node(i).Color = this.ColorMap(i,:);            % Set node color to colormap at that position in the array
      ...
      % Calculate line widths based on values of s (stored in v).   %% This
      % will actually need to calculate colors based on values of s (stored
      % in v)
      
      lineWidth  = 0.5;                 % Replaced the following code to create uniform line widths
%      minLineWidth  = 0.5;
%       lineWidthCoef = 5;
%       lineWidth = v./max(v);
        ...
        if row(i) ~= col(i)
          if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0 
            ...
            this.Node(row(i)).Connection(end+1) = line(...
              ...
              [u(2);v(2)],...
              'LineWidth', lineWidth,...
              'Color', this.ColorMap(row(i),:),... % This is where to set the line color !~!!!!!!!!!!!!
              'PickableParts','none');
